...
tf.app.flags.DEFINE_string("img_dir", "./result", "image result")
tf.app.flags.DEFINE_string("name", "result.txt", "image name")
...
def train_epoch(model, sess, X, y, loss_list): # Training Process
    loss, acc = 0.0, 0.0
...
with tf.Session() as sess:
    ...
    if FLAGS.is_train:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            train_acc, train_loss = train_epoch(mlp_model, sess, X_train, y_train, loss_plot_list)  # Complete the training process
            ...
            acc_plot_list.append(float(val_acc))
        ...
        with open(FLAGS.img_dir + "/acc_" + FLAGS.name, 'w') as json_file:
            json_file.write(json.dumps(acc_plot_list, indent=4))
        with open(FLAGS.img_dir + "/loss_" + FLAGS.name, 'w') as json_file:
